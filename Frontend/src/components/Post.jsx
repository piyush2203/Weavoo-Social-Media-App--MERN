...
const Post = ({post}) => {
    ...
    const likeOrDislikeHandler = async () => {
      try {
          ...
          const res = await axios.get(`https://weavoo.onrender.com/api/v1/post/${post._id}/${action}`, { withCredentials: true });
  ...
  const commentHandler = async () => {
    ...
    try {
        const res = await axios.post(`https://weavoo.onrender.com/api/v1/post/${post._id}/comment`, { text }, {
            headers: {
...
const bookmarkHandler = async()=>{
  try {
    const res = await axios.get(`https://weavoo.onrender.com/api/v1/post/${post?._id}/bookmark`, {withCredentials:true} );
    if(res.data.success){
      toast.success(res.data.message)
...
const handleShare = async () => {
  try {
    ...
    const deletePostHandler = async()=>{
      try {
        const res = await axios.delete(`https://weavoo.onrender.com/api/v1/post/delete/${post._id}`, {withCredentials:true});
        if(res.data.success){
